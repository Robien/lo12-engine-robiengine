...
class CameraFPSEvent : public CB_Interraction
{
public:
	CameraFPSEvent(CameraFPS* camera) : _camera(camera)
	{
		_mouseState = -1;
	}
	virtual ~CameraFPSEvent(){}
public:
	...
	virtual void eventsPassiveMotionMouse(int x, int y)
	{
		...
		_camera->tourner_tete(1, ((float)_posCurseurX-x)/20);	
		_camera->tourner_tete(2, ((float)_posCurseurY-y)/20);	
...
CameraFPS::CameraFPS()
{
	Interactions::get()->addEventCallBack(new CameraFPSEvent(this));
	resetVue();
}
...
/****************************************************************************/
void CameraFPS::affiche()						//applique l'observateur en cours //après modification de la variable typ_obs
{
	glMatrixMode(GL_MODELVIEW);	
	glLoadIdentity();
...
/****************************************************************************/
void CameraFPS::rotationZ_obs(double angle)   //rotation de l'observateur autour de l'axe Z de la scene
{	
	double x, y;
...
/****************************************************************************/
void CameraFPS::tourner_tete(int dir, double angle)
{
...
/****************************************************************************/
void CameraFPS::zoom(double pas) //permet d'avancer ou de reculer dans la scene
{
...
/****************************************************************************/
void CameraFPS::translation_obs(int dir, double pas) //déplace l'observateur vers la droite, la gauche, le bas ou le haut
{
...
/****************************************************************************/
void CameraFPS::rotation3D(double angle, double* axeVect, double* axePt, double* monPt, double* result)
